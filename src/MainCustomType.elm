...
normaliseTypeAlias (WithIdentifierMapping identifierMapping typeAlias) =
    let
        (WithIdentifierMapping identifierMapping1 name) =
            normaliseString (WithIdentifierMapping identifierMapping typeAlias.name)
    in
    WithIdentifierMapping
        identifierMapping1
...
normaliseString (WithIdentifierMapping (IdentifierMapping mapping uniqueInt) unNormalised) =
    let
        ...
        nextIdentifierMapping =
            Dict.insert (Node.value unNormalised) normalised mapping
    in
    WithIdentifierMapping
