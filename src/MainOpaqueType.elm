...
normaliseElmFile fileWithIdentifierMapping = 
    let
        elmFile = IdentifierMapping.value fileWithIdentifierMapping
        normalizedDeclarationsWithIdentifierMapping = List.foldl normaliseDeclarations (IdentifierMapping.initialize []) elmFile.declarations
    in
...
normaliseDeclarations unNormalizedDeclaration normalisedDeclarationsWithIdentifierMapping =
    let
        normalisedDeclarations = IdentifierMapping.value normalisedDeclarationsWithIdentifierMapping
        unNormalizedNodeDeclarationWithIdentifierMapping = IdentifierMapping.map (\_ -> unNormalizedDeclaration) normalisedDeclarationsWithIdentifierMapping
        range = IdentifierMapping.value unNormalizedNodeDeclarationWithIdentifierMapping |> Node.range 
        unNormalizedDeclarationWithIdentifierMapping = IdentifierMapping.map Node.value unNormalizedNodeDeclarationWithIdentifierMapping 
        normalizedDeclarationWithIdentifierMapping = normaliseDeclaration unNormalizedDeclarationWithIdentifierMapping
        normalizedNodeDeclaration = IdentifierMapping.map (Node.Node range) normalizedDeclarationWithIdentifierMapping
    in
        IdentifierMapping.map 
...
normaliseDeclaration declarationWithIdentifierMapping =
        let
            declaration = IdentifierMapping.value declarationWithIdentifierMapping
        in
        case declaration of
...
normaliseTypeAlias typeAliasWithIdentifierMapping =
    let
        typeAlias = IdentifierMapping.value typeAliasWithIdentifierMapping
        nameWithIdentifierMapping = (IdentifierMapping.map (\t -> t.name) typeAliasWithIdentifierMapping) |> normaliseNodeString
    in
        IdentifierMapping.map
            (
                \_ ->
...
normaliseNodeString unNormalizedNodeStringWithNormalizer =
    let
        nodeString = IdentifierMapping.value unNormalizedNodeStringWithNormalizer
        range = Node.range nodeString
    in
